...
impl MultiAnimationManager {
    ...
    pub fn well_lit(anim: AnimationManager) -> Self {
        let mut lighting_layers = RenderLayers::none();
        if anim
            .render_layers
            .intersects(&BgSpriteCamera::render_layers())
        {
            lighting_layers = lighting_layers.union(&BgLightCamera::render_layers());
        }
        if anim
            .render_layers
            .intersects(&SpriteCamera::render_layers())
        {
            lighting_layers = lighting_layers.union(&LightCamera::render_layers());
        }
        let mut light = anim.clone().with_render_layers(lighting_layers);
